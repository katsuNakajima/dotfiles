...
(use-package 
  clang-format 
  :config (bind-key "C-c r" 'clang-format-region) 
...
(use-package 
  projectile 
  :config (defun projectile-project-find-function (dir) 
            (let* ((root (projectile-project-root dir))) 
              (and root 
                   (cons 'transient root)))) 
  (with-eval-after-load 'project (add-to-list 'project-find-functions
...
(use-package 
  ...
  (use-package 
    ...
    :preface (defun ladicle/toggle-lsp-ui-doc () 
               (interactive) 
               (if lsp-ui-doc-mode (progn (lsp-ui-doc-mode -1) 
...
(use-package 
  ...
  :custom-face (git-gutter:modified ((t 
                                      (:background "#f1fa8c")))) 
  (git-gutter:added    ((t 
                         (:background "#50fa7b")))) 
...
(use-package 
  ...
  :custom-face (show-paren-match ((nil 
                                   (:background "#44475a" 
                                                :foreground "#f1fa8c")))) 
...
This function is intended to be used in parallel with
 `pyvenv-tracking-mode'."
    ...
    (when python-shell-virtualenv-root (setq-local pyvenv-activate (directory-file-name
